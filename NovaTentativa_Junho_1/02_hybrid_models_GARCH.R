...
USE_FEATURES <- c("Sigma_GARCH") # usando só as previsões como input
N_INS <- 1500 
EPOCHS <- 50
BATCH_SIZE <- 32
...
run_rolling_forecast <- function() {
  df <- read.csv("volatilidades_previstas_completo_corrigido_GARCH.csv")
...
sigma2_hat <- preds$predicted # previsões de sigma^2
returns2 <- preds$real # retornos^2 como proxy da volatilidade

## QLIKE
valid <- !is.na(sigma2_hat) & !is.na(returns2) & sigma2_hat > 1e-8  # evitar divisão por números muito pequenos
