# -*- coding: utf-8 -*-
"""
Created on Sat Jan 25 16:05:52 2025
...
predictions_df = pd.read_csv("predictions_gas_lstm.csv")
...
plt.plot(predictions_df["Date"], predictions_df["Predicted Values"], label="Predicted Values (GAS-LSTM)", color="red", linestyle="--", alpha=0.7)
plt.xlabel("Date")
...
plt.grid()
plt.tight_layout()
plt.savefig("comparison_proxy_gas_lstm_petrobras.png")
plt.savefig("comparison_proxy_gas_lstm_petr4.pdf")
plt.show()


# Salvar o arquivo ajustado com datas
predictions_df.to_csv("predictions_gas_lstm_with_dates.csv", index=False)

## Identificando eventos extremos

# Carregar as previsões e as proxies (com datas)
predictions_df = pd.read_csv("predictions_gas_lstm_with_dates.csv")
proxy_data = pd.read_csv("petrobras_squared_returns.csv")
...
extreme_events.to_csv("gas_lstm_extreme_events.csv", index=False)