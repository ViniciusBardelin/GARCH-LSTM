...
# ———————————————————————— #
# —————— GARCH-LSTM —————— # # rolling volatility como target; log no target
# ———————————————————————— #
...
# ————— Adiciona lags de vol^2 como novas features 

df['Variance_lag1'] = df['Variance'].shift(1)
df['Variance_lag5'] = df['Variance'].shift(5)
df[['Variance_lag1','Variance_lag5']] = df[['Variance_lag1','Variance_lag5']].bfill()
...
initial_train = 1500 # treino inicial
retrain_every = 100 # retreino
feature_cols  = ['Sigma2_GARCH','Variance_lag1','Variance_lag5', 'Returns']

# ————— Prepara arrays de features, target e datas —————
features = df[feature_cols].values # shape: (N, n_features)

raw_target = df['Variance'].values + 1e-8 # para evitar log(0)
dates = df['Date']
N = len(df)
...
pd.DataFrame({
    'Date': dates_in,
    'Residual': resid_in
}).to_csv("Res/GARCH_LSTM_residuals_in_sample_T101_mean_var.csv", index=False)
...
# Salva CSV
df_pred.to_csv("DF_PREDS/GARCH_LSTM_T101_mean_var.csv", index=False)
...
# y_true como a série Parkinson OoS
y_true = df['Variance'][initial_train:].reset_index(drop=True).values
...
plt.plot(df['Date'], df['Variance'],
         label='Parkinson', color='tab:blue', linewidth=1)
