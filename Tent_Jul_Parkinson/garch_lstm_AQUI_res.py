...
for t in range(initial_train, N):
    ...
    residuals_lstm.append(residual)            # <--- ARMAZENA RESÍDUO
...
# Salva CSV
df_pred.to_csv("DF_PREDS/T591.csv", index=False)

# Separa resíduos in-sample (1500:1500+window_size) e OoS
residuals_insample = residuals_lstm[:window_size]  
residuals_oos = residuals_lstm[window_size:]  

# Exporta para CSV
pd.DataFrame({'Residuals_LSTM': residuals_insample}).to_csv("residuals_lstm_insample_591.csv")
pd.DataFrame({'Residuals_LSTM': residuals_oos}).to_csv("residuals_lstm_oos_591.csv")
...
plt.grid("--", alpha=0.4)
plt.tight_layout()
plt.savefig("Resultados/T591.pdf", format="pdf", bbox_inches="tight")
plt.show()
