...
retrain_every = 100    # retreina a cada 252 dias
...
reduce_on_plateau = ReduceLROnPlateau(
    monitor='val_qlike_loss',
    factor=0.5,
    patience=5,
    min_lr=1e-6,
    verbose=1
)
# se for usar, precisa passar `callbacks=[es, reduce_on_plateau]` no retrain
...
def build_model(input_shape):
    model = keras.models.Sequential([
        # Primeira camada BiLSTM com 64 unidades (32 em cada direção)
        Bidirectional(LSTM(64, return_sequences=True), input_shape=input_shape),
        #keras.layers.Dropout(0.2),
        
        # Segunda camada BiLSTM com 32 unidades (16 em cada direção)
...
preds, pred_dates, residuals_lstm = [], [], []
residuals_insample_full = []  # Para os 1500 valores in-sample
residuals_oos_full = []   
for t in range(initial_train, N):
    ...
    if (t - initial_train) % retrain_every == 0:
        ...
        model.fit(
            ...
            validation_split=0.1,
            callbacks=[es],
            # callbacks=[es, reduce_on_plateau],
            verbose=1
            )
    ...
    # --- CÁLCULO DO RESÍDUO DO GARCH-LSTM ---
    residual = df['Returns'].iloc[t] / p_orig  # r_t / σ_{LSTM,t}
    # Classifica como in-sample ou OoS
    if t < initial_train:
        residuals_insample_full.append(residual)  # Pré-OoS
    else:
        residuals_oos_full.append(residual)       # OoS
...
# Salva CSV
df_pred.to_csv("DF_PREDS/T592.csv", index=False)

# Exporta CSVs com tamanhos corretos
pd.DataFrame({'Residuals_LSTM': residuals_insample_full[:1500]}).to_csv("Res/residuals_lstm_insample_FULL_592.csv")
pd.DataFrame({'Residuals_LSTM': residuals_oos_full}).to_csv("Res/residuals_lstm_oos_FULL_592.csv")
...
# 3) Valores válidos (evita zeros/NaN)
eps = 1e-8
...
plt.grid("--", alpha=0.4)
plt.tight_layout()
plt.savefig("Resultados/T592.pdf", format="pdf", bbox_inches="tight")
plt.show()
