...
pd.DataFrame({
    'Date': dates_in,
    'Residual': resid_in
}).to_csv("Res/LSTM_residuals_in_sample_T101.csv", index=False)
...
# Salva CSV
df_pred.to_csv("DF_PREDS/T101.csv", index=False)
...
# Previsões GARCH e GARCH-LSTM, alinhadas com y_true
...
plt.grid("--", alpha=0.4)
plt.tight_layout()
plt.savefig("Resultados/T101.pdf", format="pdf", bbox_inches="tight")
plt.show()
