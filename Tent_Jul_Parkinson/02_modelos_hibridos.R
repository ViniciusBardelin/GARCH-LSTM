...
# === PARÂMETROS GLOBAIS === #
WINDOW_SIZE <- 22 # tamanho da janela dentro da LSTM
NUM_RUNS <- 1
MODELS <- c("garch")
TARGET <- "Parkinson"
...
tst <- read.csv("volatilidades_previstas_completo_corrigido_GARCH_1_1.csv")

plot(tst$Sigma_GARCH, type = 'l')
...
run_rolling_forecast <- function() {
  ...
  for (model_type in MODELS) {
    ...
    for (run in 1:NUM_RUNS) {
      ...
      for (start in 601:total_janelas) {
...
# MÉTRICAS
df1 <- read.csv("rolling_preds/garch/run_1_partial_up_to_600.csv")
df2 <- read.csv("rolling_preds/garch/run_1_partial_up_to_200.csv")
df3 <- read.csv("rolling_preds/garch/run_1_partial_up_to_1132.csv")
#df4 <- read.csv("rolling_preds_18/garch/run_1_partial_up_to_2267.csv")

df_all <- bind_rows(df1)
df_all <- tail(df_all, 1132)

sigma2_hat <- df_all$predicted # previsões
returns2 <- df_all$real # proxy

# QLIKE
valid <- !is.na(sigma2_hat) & !is.na(returns2) & sigma2_hat > 1e-8
qlike <- mean(log(sigma2_hat[valid]) + (returns2[valid] / sigma2_hat[valid]))
print(paste("QLIKE:", qlike))

# MSE
mse_val <- mse(returns2, sigma2_hat)
cat("MSE:", mse_val, "\n")

# Gráfico
preds_long <- df_all %>%
  select(window_start, Predito = predicted, Real = real) %>%
  pivot_longer(cols = c(Predito, Real), names_to = "Série", values_to = "Valor")

ggplot(preds_long, aes(x = window_start, y = Valor, color = Série)) +
  geom_line(linewidth = 0.8, alpha = 0.85) +
  scale_color_manual(values = c("Real" = "black", "Predito" = "blue")) +
  labs(
    title = "GARCH-LSTM vs Retornos²",
    x = "Janela (início da rolling window)",
    y = expression(hat(sigma)^2~" vs "~r^2),
    color = NULL
  ) +
