...
# 1) Previsões in-sample (log-scale)
p_log_in = model.predict(X_tr, verbose=0)            # shape = (initial_train-window_size, 1)

# 2) Reverte log + scaler_y para obter sigmâ original
p_in = scaler_y.inverse_transform(p_log_in)          # ainda em log
p_in = np.exp(p_in).flatten() - 1e-8                 # sigmâ_t^LSTM

# 3) Retornos correspondentes (in-sample)
rets_in = df['Returns'].values[window_size:initial_train]

# 4) Resíduos padronizados ε_t = r_t / sigmâ_t
...
pd.DataFrame({
    'Date':     dates_in,
    'Residual': resid_in
}).to_csv("Res/LSTM_residuals_in_sample_75.csv", index=False)
...
df_pred.to_csv("DF_PREDS/T75.csv", index=False)
...
plt.tight_layout()
plt.savefig("Resultados/T75.pdf", bbox_inches="tight")
plt.show()
